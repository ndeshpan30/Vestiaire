...
export default async function ClosetPage() {
  ...
  // 1. Wrap data fetching in a try/catch block
  try {
    ...
    console.error('[ClosetPage] Global data fetching error:', fetchErr);
    garments = [];
  }

  // 2. Safe property calculations with fallbacks & optional chaining
  const totalItems = garments.length;
  const topsCount = garments.filter((item) => {
    const category = item?.category || item?.taxonomy_path || '';
    return ['Top', 'TOPS', 'Shirts', 'Blouse', 'Jacket', 'Outerwear'].some((c) =>
  ...
  const bottomsCount = garments.filter((item) => {
    const category = item?.category || item?.taxonomy_path || '';
    return ['Bottom', 'BOTTOMS', 'Trousers', 'Pants', 'Skirt'].some((c) =>
      category.toLowerCase().includes(c.toLowerCase())
    );
  }).length;
  ...
  const shoesCount = garments.filter((item) => {
    const category = item?.category || item?.taxonomy_path || '';
    return ['Shoes', 'Footwear', 'Loafers', 'Boots'].some((c) =>
      category.toLowerCase().includes(c.toLowerCase())
    );
  }).length;
        ...
        {garments.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {garments.map((garment, index) => {
              // 3. Render try/catch & safe property access per garment
              try {
                const id = garment?.id || `garment-fallback-${index}`;
                const category = garment?.category || 'Uncategorized';
                const title = garment?.title || garment?.subcategory || 'Garment Item';
                const color = garment?.color || garment?.primary_color || 'Unknown';
                const material = garment?.material || garment?.material_guess || null;
                const formality = typeof garment?.formality === 'number' ? garment.formality : 5;
                const imageUrl = garment?.image_url || garment?.image_path || '/placeholder.png';
                const seasonList = Array.isArray(garment?.season)
                  ? garment.season
                    ...
                    <div className="p-4 flex flex-col justify-between flex-1">
                      ...
                      <div className="pt-3 border-t border-[#EEEEEE] flex items-center justify-between">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {seasonList.length > 0 && (
                            <span className="px-2 py-0.5 bg-[#F5F5F5] border border-[#E5E5E5] text-[#737373] text-[10px] font-medium rounded">
                              {seasonList.join(', ')}
                            </span>
                          )}
                        </div>
                ...
                console.error('Error rendering garment:', garment, renderErr);
                return null;
              }
            })}
          </div>
